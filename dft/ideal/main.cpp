...
double circle(double x, double y, double D){
    if(sqrt((x*x) + (y*y)) < D)
        return 1.0;
    else
        return 0.0;
...
void createIdealFilter(Mat &dft_Filter, double cutoffInPixels, bool inv)
{
    ...
    for(int u = 0; u < ghpf.rows; u++)
    {
        ...
        {
            ghpf.at<float>(u, v) = circle(u - center.y, v - center.x, cutoffInPixels*0.5);
        }
    }
...
{ 
    ...
    while(true){
        ...
        createIdealFilter(filter, cut_off, true);
